...
try {
    ...
    if (-not $SkipLiveGemini) {
        & $python -m hotel_bot.llm
        if ($LASTEXITCODE -ne 0) { throw "Live Gemini smoke failed." }
    }
...
Write-Host "Step 8 verification passed."
